...
相机输入仍为 `head_camera` RGB。相机采集和 HDF5 原图是 `640×360`，当前
Stage 2 训练加载器使用了 `320×240`；因此真实推理按照
`config.real_vla.yaml` 的 `model.input_image_size: [320, 240]`，在送入 Qwen
之前使用 OpenCV `INTER_LINEAR` 执行相同缩放。该设置仅用于匹配当前
checkpoint，不会修改采集分辨率或已有数据。
...
4. `s`：停止模型动作并保持当前位置。
5. `h`：当前轮停止后，低速回到下一轮的 L1～L6 初始位置；L7保持当前开度。
...
一轮的推荐循环是：`p → r → s → h → p → r`。如果已经达到 40 秒自动停止，
可以直接按 `h`。`h` 会先停止动作预取和运行控制线程，再独占执行回位；回位后会
恢复安全保持线程，并强制要求重新 `p`，不会复用上一轮的动作块。整个 `h`
流程不会向L7发送打开、闭合或反馈位置命令。
